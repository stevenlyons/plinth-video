...
1. **Beacon enrichment** — Add `playhead_ms` to `pause`, `ended`, and `completed` beacons so the server always knows the viewer's last known position at every natural session boundary, even if the final heartbeat was never received.
...
- Server can restore a viewer's playback position after they exit by reading the most recent `pause`, `ended`, or `completed` beacon.
- Server can determine whether a video was "completed" by checking for a `completed` beacon (natural end) vs `ended` beacon (session torn down).
...
### `completed` and `ended` beacons

Add `playhead_ms` to all `completed` and `ended` beacons. This captures the final position of the session regardless of how it ended.

These beacons are emitted from three paths:
- `Playing → Ended` (`PlayerEvent::Ended` — video played to natural end) → `completed`
- `Paused → Idle` (`PlayerEvent::Destroy` — session torn down while paused) → `ended`
- `destroy()` from any active state (platform teardown, e.g. app backgrounded) → `ended`
...
Add `playhead_ms` to the shared `Beacon.properties` section so it is documented at the top level, and add a new `allOf` condition that flags it as present (but not required) on `pause`, `ended`, and `completed`:
...
"playhead_ms": {
  "type": "integer",
  "minimum": 0,
  "description": "Current playhead position in the content timeline in milliseconds. Required on heartbeat, pause, ended, and completed — capturing the last known position at each session boundary."
}
```
...
{
  "if": {
    "properties": { "event": { "enum": ["pause", "ended", "completed"] } },
    "required": ["event"]
  },
  "then": {
    "properties": {
...
The existing `heartbeat` condition retains `"required": ["playhead_ms"]`. The `pause`, `ended`, and `completed` condition omits `required` because `playhead_ms` defaults to `0` if `set_playhead` was never called, but is always present.
...
| 3 | Natural end carries `playhead_ms` | `set_playhead(120000)`, emit `Ended`, assert `completed` beacon `playhead_ms == Some(120000)` |
| 4 | Paused → Destroy carries `playhead_ms` | Reach Paused, `set_playhead(30000)`, emit `Destroy`, assert `ended` beacon `playhead_ms == Some(30000)` |
| 5 | `destroy()` from Playing carries `playhead_ms` | Reach Playing, `set_playhead(15000)`, call `destroy()`, assert `ended` beacon `playhead_ms == Some(15000)` |
...
| File | Change |
|---|---|
| `crates/plinth-core/src/session.rs` | Set `b.playhead_ms` on `pause`, `ended`, and `completed` beacons |
| `docs/reference/beacon-payload.schema.json` | Add `playhead_ms` to `Beacon.properties`; add `allOf` condition for `pause`/`ended`/`completed` |
